...
- [x] **QURY-01**: User can find the definition of a symbol (name → file:line location)
- [x] **QURY-02**: User can find all references to a symbol across the codebase
- [x] **QURY-03**: User can get the impact/blast radius of changing a symbol (transitive dependents)
- [x] **QURY-04**: User can detect circular dependencies in the import graph
- [x] **QURY-05**: User can get a 360-degree context view of a symbol (definition, callers, callees, type usage)

### Integration

- [x] **INTG-01**: Tool runs as an MCP server over stdio, exposing graph queries as tools to Claude Code
- [x] **INTG-02**: MCP tool responses use a token-optimized compact format (target: ~60% fewer tokens than verbose JSON)
- [x] **INTG-03**: MCP tool descriptions are concise (under 100 tokens each) to minimize per-turn overhead
- [x] **INTG-04**: Tool runs as a background daemon with file watching, re-indexing incrementally on file changes
- [x] **INTG-05**: Incremental re-index completes in under 100ms for single-file changes
- [x] **INTG-06**: Tool provides CLI commands: index (full index), query (symbol lookup), impact (blast radius), stats (overview)

### Performance

- [x] **PERF-01**: Full index of a 10K-file TypeScript codebase completes in under 30 seconds
- [x] **PERF-02**: Tool uses parallel parsing (rayon) to utilize multi-core CPUs
- [x] **PERF-03**: Background daemon uses less than 100MB RSS for typical projects (<10K files)
