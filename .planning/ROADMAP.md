...
**Success Criteria** (what must be TRUE):
  1. `code-graph find <symbol> <path>` returns the file path and line number of the symbol's definition
  2. `code-graph refs <symbol> <path>` returns all files and locations that reference the symbol
  3. `code-graph impact <symbol> <path>` returns the transitive set of files and symbols that would break if the symbol changed
  4. `code-graph circular <path>` reports all circular dependency cycles in the import graph
  5. `code-graph stats <path>` prints an overview of file count, symbol count, and module structure
**Plans:** 3/3 plans complete
Plans:
- [ ] 03-01-PLAN.md — Query infrastructure, find command, and stats command (QURY-01, INTG-06)
- [ ] 03-02-PLAN.md — References, impact/blast radius, and circular dependency detection (QURY-02, QURY-03, QURY-04, INTG-06)
- [ ] 03-03-PLAN.md — 360-degree context command completing all CLI queries (QURY-05, INTG-06)
...
| 5. Watch Mode & Persistence | 1/3 | In Progress|  |
